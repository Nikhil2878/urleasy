...
export const verifyEmailToken = async (req, res) => {
  ...
  // to find email- update the is_email_valid validityState
...
//Validate token authentication, expiration, and match with a previously issued token.
//If valid , get new password from request body and validate using a schema(Zod) for complexity
...
export const postSetPassword = async (req, res) => {
  ...
  const { newPassword } = data;
  // console.log("Ye hai new password : ", newPassword);
  const user = await findUserById(req.user.id);
  if (user.password) {
    req.flash(
      "errors",
